...
function createWasi(args: string[]) {
  ...
  const functions = {
    ...
    args_sizes_get(
      ...
      view.setUint32(
        bufferSizePointer,
        encodedArgs.reduce((size, argument) => size + argument.length + 1, 0),
        true,
      );
      return wasiSuccess;
    },
    ...
    random_get(bufferPointer: number, bufferLength: number): number {
      const target = new Uint8Array(memory.buffer, bufferPointer, bufferLength);
      for (let offset = 0; offset < target.length; offset += 65_536) {
        crypto.getRandomValues(
          target.subarray(offset, Math.min(offset + 65_536, target.length)),
        );
      }
      return wasiSuccess;
    },
    ...
    fd_write(
      ...
      if (output)
        (fd === 2 ? console.error : console.log)(output.replace(/\n$/, ''));
      return wasiSuccess;
    },
...
export class AutomatonEngine {
  ...
  get grid(): GridView | null {
    ...
    if (!this.view || this.view.cells.buffer !== this.exports.memory.buffer) {
