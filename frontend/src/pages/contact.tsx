...
const contactFormStore = store({
    status: 'idle' as 'idle' | 'loading' | 'success' | 'error',
    message: '',
})
    .action('submit', async (event: SubmitEvent, { set }) => {
        const form = event.target as HTMLFormElement;
        set({ status: 'loading' });

        try {
            const payload = emailSchema.parse(extractFormData(form));
    ...
    return (
        <div class='flex flex-col items-start gap-3 sm:flex-row sm:items-center sm:justify-between'>
            <div aria-live='polite' class='min-h-6 text-sm'>
                {status === 'error' && (
                    <p class='text-[hsl(2_62%_54%)]'>{message}</p>
                )}
                {status === 'success' && (
                        ...
                        <form class='flex flex-col gap-6' novalidate>
                            <div class='grid gap-6 sm:grid-cols-2'>
                                <Field
                                    ...
                                    validate={(value) =>
                                        emailSchema.shape.full_name.safeParse(
                                                value,
                                            ).success
                                            ? null
                                            : labels.nameError}
                                ...
                                <Field
                                    ...
                                    validationMode='onSubmit'
                                    validate={(value) =>
                                        emailSchema.shape.email.safeParse(value)
                                                .success
                                            ? null
                                            : labels.emailError}
                                >
                            ...
                            <Field
                                ...
                                validationMode='onSubmit'
                                validate={(value) =>
                                    emailSchema.shape.content.safeParse(value)
                                            .success
                                        ? null
                                        : labels.messageError}
                            >
